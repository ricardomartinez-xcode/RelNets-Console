...
test('Console OAuth starts with PKCE and the canonical user API resource', () => {
  ...
  assert.equal(url.origin, 'https://auth.relead.com.mx');
  assert.equal(url.pathname, '/oauth/authorize');
  assert.equal(url.searchParams.get('client_id'), 'relead-console');
  assert.equal(url.searchParams.get('redirect_uri'), 'https://console.relead.com.mx/auth/callback');
...
test('Console API and MCP publish distinct OAuth resources', () => {
  assert.equal(USER_API_RESOURCE, 'https://console.relead.com.mx/v2');
  assert.equal(USER_MCP_RESOURCE, 'https://console.relead.com.mx/mcp');
  assert.notEqual(USER_API_RESOURCE, USER_MCP_RESOURCE);
  assert.deepEqual(resourceMetadata(USER_API_RESOURCE, ['relnet.profile.read']).authorization_servers, ['https://auth.relead.com.mx']);
});
