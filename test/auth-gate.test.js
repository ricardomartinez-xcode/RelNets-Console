...
test('Console OAuth starts on the Console-owned issuer with PKCE and billing scopes', () => {
  ...
  assert.match(url.searchParams.get('scope') || '', /billing\.read/);
  assert.match(url.searchParams.get('scope') || '', /billing\.write/);
  assert.match(url.searchParams.get('scope') || '', /offline_access/);
});

test('Console API and MCP publish distinct resources owned by the Console issuer', () => {
  ...
  assert.deepEqual(resourceMetadata(USER_API_RESOURCE, ['relnet.profile.read']).authorization_servers, ['https://console.relnets.com']);
});
